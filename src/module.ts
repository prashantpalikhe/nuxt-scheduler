  ...
  setup(options, nuxt) {
    ...
    addTemplate({
      ...
      getContents: () =>
        [
          ...
          `  const useScheduler: typeof import('${resolve(
    ...
    nuxt.hook("prepare:types", (options) => {
      options.references.push({
        path: resolve(nuxt.options.buildDir, "types/scheduler.d.ts"),
      });
    });

    const schedulerPath = resolve(
      nuxt.options.rootDir + "/server/app/scheduler.ts"
    );
    ...
    if (scheduleFileExists) {
      ...
      logger.error(
        "nuxt-scheduler not active: cannot find ~/server/app/scheduler.ts"
      );
    }
  },
});
